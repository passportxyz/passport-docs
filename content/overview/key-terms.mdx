...
See our [quick start guide](/building-with-passport/stamps/passport-api/quick-start-guide) to learn how to gate your project.
...
```bash
curl --request GET 'https://api.passport.xyz/registry/stamps/{address}?include_metadata=true'
```
...
[Individual Verifications](/building-with-passport/individual-verifications) are privacy-preserving identity checks that use zero-knowledge proofs to verify specific attributes about a user without exposing personal data. Unlike Stamps which aggregate multiple signals into a composite score, Individual Verifications provide targeted proofs for use cases requiring stronger identity guarantees or regulatory compliance.
...
- **Proof of Clean Hands** - Sanctions and PEP list verification for AML compliance

Upon successful verification, users receive a non-transferable SoulBound Token (SBT) that proves their verification status.



### Zero-Knowledge Proofs (ZKP)

A zero-knowledge proof (ZKP) is a cryptographic method that allows one party to prove a statement is true without revealing any information beyond the statement's validity. For example, a user can prove they passed government ID verification without disclosing their name, ID number, or any other personal data. In Human Passport, ZKPs power [Individual Verifications](/building-with-passport/individual-verifications/introduction) — users generate compact cryptographic proofs client-side, which are verified on-chain as attestations. The underlying personal data is never disclosed to the application requesting the proof. See [How Zero-Knowledge Proofs Work](/building-with-passport/individual-verifications/introduction#how-zero-knowledge-proofs-work) for a developer-oriented explanation.



### SoulBound Tokens (SBT)

SoulBound Tokens are non-transferable blockchain tokens that represent identity credentials or achievements. Unlike regular tokens or NFTs, SBTs cannot be transferred or sold - they are permanently bound to the wallet that received them. Human Passport uses SBTs to represent completed [Individual Verifications](/building-with-passport/individual-verifications). When a user completes a verification (such as government ID or biometrics), they receive an SBT on their chosen blockchain (Optimism or Stellar) that proves their verification status. SBTs expire after one year and can be renewed by completing the verification process again.
