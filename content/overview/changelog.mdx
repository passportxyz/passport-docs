...
We added support for linked accounts in the Stamps API. Users can now link multiple wallet addresses under a single canonical WaaP identity, unifying their Stamps and scores across all linked addresses. API responses now include three new fields: `canonical_address`, `linked_addresses`, and `eligible_for_campaign`. This enables sybil deduplication across linked wallets for campaigns and reward programs.

Learn more in our [account linking documentation](/building-with-passport/stamps/major-concepts/account-linking). See also [passportxyz/passport-scorer#931](https://github.com/passportxyz/passport-scorer/pull/931).
