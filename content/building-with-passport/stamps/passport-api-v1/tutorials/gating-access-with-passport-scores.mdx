...
Now, create a new file in `my-passport-app` and add the following to set the Stamps API Key and Scorer ID values (if you need a refresher on how to generate a Scorer ID, revisit the [Quick Start guide](/building-with-passport/stamps/passport-api-v1/quick-start-guide)):
