---
title: Stamps API v1 -- API reference (Retired)
description: Reference documentation for the Stamps API v1 - This API version is retired
noindex: true
---
...
To access the Stamps API, you will need a [Scorer ID and an API key](/building-with-passport/stamps/passport-api/getting-access).
...
```bash
curl --request GET 'https://api.passport.xyz/registry/signing-message' \
...
* **Data dictionary:** We have put together a [data dictionary](/building-with-passport/stamps/major-concepts/data-dictionary) that you can use to better understand each field that delivers with the response payloads from the Stamps API endpoints.
...
curl --request GET \
    --url https://api.passport.xyz/registry/signing-message \
...
```bash filename="Sample request" copy
curl --request POST \
  --url https://api.passport.xyz/registry/submit-passport \
  --header 'X-API-KEY: {API KEY}' \
...
The name in the parenthesis represents what [type of Scorer](/building-with-passport/stamps/passport-api/getting-access#types-of-scorers) you are using.
...
```bash filename="Sample request" copy
curl --request GET \
    --url https://api.passport.xyz/registry/score/{scorer_id}/{address} \
    --header 'X-API-KEY: {API KEY}'
```
...
```bash filename="Sample request" copy
curl --request GET \
    --url https://api.passport.xyz/registry/score/{scorer_id} \
    --header 'X-API-KEY: {API KEY}'
```
...
```bash filename="Sample request" copy
curl --request GET \
    --url https://api.passport.xyz/registry/score/{scorer_id}?address={address}?last_score_timestamp_get=2023-07-20T19%3A54%3A44.463335%2B00%3A00 \
    --header 'X-API-KEY: {API KEY}'
```
...
```bash filename="Sample request" copy
curl --request GET \
    --url 'https://api.passport.xyz/registry/stamps/{address}?include_metadata=true' \
    --header 'X-API-KEY: {API KEY}'
```
...
```bash filename="Sample request" copy
curl --request GET \
    --url https://api.passport.xyz/registry/stamp-metadata \
    --header 'X-API-KEY: {API KEY}'
```
...
```bash filename="Sample request" copy
curl --request GET \
    --url https://api.passport.xyz/registry/gtc-stake/{address}/{round_id} \
    --header 'X-API-KEY: {API KEY}'
```
