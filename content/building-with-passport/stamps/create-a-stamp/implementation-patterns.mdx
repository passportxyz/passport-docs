...
| **Onchain** | Blockchain verification, token ownership | Binance BABT, ETH balance | High |
| **Custom API** | Proprietary systems, complex workflows | Guild.xyz, custom KYC | High |
...
<Callout type="info">
**Use Case:** Verifying blockchain-based credentials, token ownership, or onchain activity.
</Callout>
...
export class YourOnChainPlatform extends Platform {
  ...
  banner = {
    heading: "Verify Your Onchain Activity",
    content: (
      <div>
        <p>This Stamp verifies your onchain verification status.</p>
        <strong>Requirements:</strong>
        <ul>
        ...
        <ol>
          ...
          <li>Click "Verify" to check your onchain status</li>
  ...
  // For onchain verification, payload comes from wallet context
...
export class YourOnChainProvider implements Provider {
  ...
  async verify(payload: RequestPayload): Promise<VerifiedPayload> {
    ...
    try {
      ...
      return {
        valid: false,
        errors: [`Unable to verify onchain status: ${error.message}`],
      };
    }
  }
    ...
    providers: [
      {
        ...
        name: "YourProviderName_verified",
      },
    ],
  },
];
