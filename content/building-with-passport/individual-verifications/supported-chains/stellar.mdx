...
| **ePassport** | `0xf2ce248b529343e105f7b3c16459da619281c5f81cf716d28f7df9f87667364d` | [id.human.tech/epassport](https://id.human.tech/epassport) |
...
ePassport is a distinct verification type available on Stellar. It uses the NFC chip in modern passports for cryptographic identity verification.

- **Distinct circuit ID**: ePassport has its own circuit ID (`0xf2ce...364d`), separate from Government ID
- **Free verification**: No fee required for ePassport verification
- **NFC-based**: Reads the passport's NFC chip for cryptographic proof
- **One SBT per passport**: Each passport can only mint one SBT

<Callout type="tip">
Privacy: users can pay from a different wallet than the SBT recipient. This prevents linking the funding wallet to the verified identity.
</Callout>
