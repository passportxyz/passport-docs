...
To use Individual Verifications with Snapshot, configure the ["api" strategy](https://github.com/snapshot-labs/snapshot-strategies/tree/master/src/strategies/api) with the following parameters:

```json filename="Government ID Sybil Resistance"
{
...
```json filename="Phone Sybil Resistance"
{
  "api": "https://api.holonym.io",
  "symbol": "",
  "decimals": 0,
...
<Callout type="info">
We recommend using the default action ID `123456789`. See [Custom Action IDs](/building-with-passport/individual-verifications/major-concepts/action-ids) for information on using custom action IDs.
</Callout>
