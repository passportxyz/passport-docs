...
For example, you should extract the position indexes of each `1` value in the `providers` bitmap, then look up the `Providers` at those same indexes in the providers mapping, found on the [Gitcoin Github](https://github.com/passportxyz/eas-proxy/blob/main/deployments/providerBitMapInfo.json).
