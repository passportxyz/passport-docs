...
| `getScore(userAddress)` (0xdaadd662)    | Returns a specified address's score based on the specified scorer ID (also known as community ID). Divide the value by 10000 to get the user’s score.  <br />This function will read the user’s score first from the cache in the `GitcoinResolver` and fall back to reading the score from the EAS [Human Passport attestation schema](../attestation-schema#human-passport-attestation-schema). This method will revert with an error is a valid score is not found.<br /><br />Partners who are using [Custom Passport](/building-with-passport/custom-passport) can use this method to retrieve a custom score from a user                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
