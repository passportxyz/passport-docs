...
```bash
curl --request GET \
    --url 'https://api.passport.xyz/v2/models/score/{address}?models=aggregate' \
...
```bash
curl --request GET \
    --url 'https://api.passport.xyz/v2/models/score/{address}?models=arbitrum' \
    --header 'Content-Type: application/json' \
    --header 'X-API-KEY: {API KEY}'
    ```
