...
* Assuming the user didn’t pass primary verification, submit the user’s address to the Passport API [`POST /registry/submit-passport`](../passport-api/api-reference#submit-and-retrieve-latest-score-for-a-single-address), which will score (or refresh the score) of the user, and deliver either the user’s [unique humanity or binary score](../major-concepts/scoring-mechanisms).
