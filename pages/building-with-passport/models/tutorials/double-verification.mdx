...
Replace the contents of `app/page.tsx` with the following boilerplate code (this includes all the very basic logic to render a page and connect a wallet to the app and the logic required to check a Passport - if you need a refresher on the Stamps API logic you can visit our dedicated [tutorials](../../passport-api/tutorials.mdx)):
