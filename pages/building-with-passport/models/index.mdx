...
* **Modular verification:** Using the MBD API along with the [Stamp-based verification](/building-with-passport/stamps/passport-api) approach made available via the Stamps API can enable you to provide multiple verification options that reduce user friction for the majority of users while offering an additional verification tool for users who don’t pass the initial check.
...
One of our recomended flows is to use double verification with both the MBD API and the Stamp-based Stamps API. We have provided a [walkthrough tutorial](tutorials/double-verification) to show you how it's done.
