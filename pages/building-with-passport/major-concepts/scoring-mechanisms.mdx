...
description: Explanation of the various scoring algorithms implemented in Passport XYZ.
---
...
There are Passport XYZ built-in scoring mechanisms that can be executed on the Passport server, or you can design and implement your own.

![scoring mechanisms](public/scoring-mechanisms.png)

## Built-in scoring mechanisms

When you visit [developer.passport.xyz](https://developer.passport.xyz) and create a Scorer, you are presented with four options: 
...
on the Passport server.

### Unique Humanity

Unique Humanity assigns a weight to each individual Stamp. The score is simply the sum of the Stamp weights. The Stamp weights are set by Passport
and can be viewed on the [Passport Github](https://github.com/passportxyz/passport-scorer/blob/main/api/scorer/settings/gitcoin_passport_weights.py).
...
You are also welcome to apply your own scoring mechanism to raw Stamp data. The built-in Scorers are executed on the Passport server. You can also 
...
the Passport server or onchain Stamps. An example of client-side custom scoring is provided in our [onchain Stamps](/building-with-passport/tutorials/integrating-onchain-stamp-data.md) tutorial.
...
On the Passport server, the Stamp that was seen *first* is treated as valid, and any duplicates that are used in other wallets later are ignored in a process known as '[deduplication](/building-with-passport/major-concepts/deduplicating-stamps)'.

If you use a built-in Scorer, deduplication is handled for you on the Passport server. However, if you choose to implement some custom algorithm, you should also
