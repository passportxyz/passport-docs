...
### How does Passport XYZ handle duplicate Stamps?
...
By default, the Passport API uses a **Last In, First Out (LIFO)** Stamp deduplication method.
...
For example, let’s say you build an application that uses the Passport API to verify the unique humanity of your users. Two Passports, “Passport A” and “Passport B,” present the same Stamp based on the same Twitter account. In this scenario, the Last-in-First-out deduplication method would only count the Stamp instance that was submitted earliest, ignoring the one that was submitted later.
