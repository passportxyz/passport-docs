...
- We've introduced a new method, `getScore` (0xdaadd662), which accepts a scorer ID (communityID) and user address, and returns the current score of a user for that scorer. This can be used by partners who are using [custom scores](./building-with-passport/custom-passport.md). At this time, there is no way to request Stamp-specific Custom Passport data from onchain. 
